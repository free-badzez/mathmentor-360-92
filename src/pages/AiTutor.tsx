...
const AiTutor = () => {
  ...
  const handleQuestionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('gemini-tutor', {
        body: {
          question: question
        }
      });
  ...
  return <div className="min-h-screen bg-background text-foreground pt-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleQuestionSubmit} className="space-y-4">
              <div className="flex gap-4">
                <Input value={question} onChange={e => setQuestion(e.target.value)} placeholder="e.g., How do I solve quadratic equations?" className="flex-1 bg-background text-foreground" />
        ...
        <div className="space-y-6">
          {chatHistory.map(message => <Card key={message.id} className={`glass-card ${message.type === 'user' ? 'bg-secondary' : 'bg-primary/5'}`}>
              <CardContent className="p-4 bg-zinc-950 hover:bg-zinc-800 ">
                <div className="flex items-start gap-3">
                  {message.type === 'user' ? <MessageCircle className="h-6 w-6 text-muted-foreground" /> : <Brain className="h-6 w-6 text-primary" />}
